...
# Define a function to handle year conditions
def filter_students_by_year(data, year):
    if isinstance(year, int):
        return data[data['Year '] == year]
    elif isinstance(year, str):
        return data[data['Year ' ].astype(str).str.contains(year, case=False, na=False)]
    else:
        return pd.DataFrame()  # Return an empty DataFrame if the year is not handled
...
second_year_students = filter_students_by_year(data, "2")
third_year_students = filter_students_by_year(data, "3")
fourth_year_students = filter_students_by_year(data, "4")
